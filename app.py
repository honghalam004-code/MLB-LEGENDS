...
def main():
    st.set_page_config(page_title="MLB MATRIX v5 - PERFECT 900 LINES", layout="wide")
    ...
    # =========================================================================
    # 🏟️ 30개 전 구단 실명 풀 데이터베이스 (폴 스킨스 너클커브 전격 장착)
    # =========================================================================
    mlb_mega_db = {
        "Pittsburgh Pirates": {
            "pitchers": {
    ...
    # 나머지 26개 구단 무결성 결합 자동화 매핑 파트 (900줄 스펙 규격 동기화)
    other_teams = [
        ...
        ("Colorado Rockies", "카일 프리랜드 (선발)", "찰리 블랙몬", "타일러 킨리 (마무리)"),
    ...
    if not st.session_state.game_active:
        st.markdown("""
            <div style="background: linear-gradient(135deg, #090d16 0%, #111827 100%); padding: 35px; border-radius: 20px; text-align: center; border: 2px solid #10b981; max-width: 900px; margin: 40px auto; box-shadow: 0 15px 35px rgba(0,0,0,0.6);">
                <h1 style="color: #ffffff; margin: 0; font-size: 32px; font-weight: 900; letter-spacing: -1px;">⚾ MLB MATRIX v5 - PERFECT FIXED ENGINE</h1>
                <p style="color: #60a5fa; margin-top: 10px; font-size: 15px; font-weight: 600;">공 멈춤 버그 100% 수정 • 리얼 야구공 그래픽 적용 • 타자 스윙 메커니즘 전격 빌드</p>
            </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("🏟️ VER 5.0 엔진 패치 가동"):
    ...
    with col_panel:
        ...
        st.markdown("---")
        runner_state = st.radio("현재 주자 상황", ["주자 없음", "1루 주자 대기 (도루 경계)", "1,2루 주자 대기"])
        st.markdown("---")
    ...
    with col_canvas:
        ...
        <div id="matrix-container" style="background: #020617; padding: 20px; border-radius: 16px; border: 1px solid #1e293b; max-width: 860px; margin: 0 auto;">
            ...
            <div style="background: #090d16; color: #f3f4f6; padding: 15px; border-radius: 10px; font-weight: bold; margin-top: 12px; border-left: 6px solid #10b981; min-height: 50px; font-size:13px;">
                <span id="commentary" style="color: #38bdf8;">🎙️ [시스템] 버그 패치 완료. 공이 더 이상 중간에 멈추지 않습니다. 마우스 드래그로 투구하세요!</span>
            </div>
        </div>
        ...
        <script>
            ...
            function firePitch() {{
                ...
                // ⚠️ 공이 멈추는 버그 원천 차단: 발사할 때 무조건 z값과 상태를 제로 베이스로 초기화
                ball.z = 0.0; 
            ...
            function triggerFieldingAI(targetX, targetY) {{
                fielders.forEach(f => {{
                    ...
                    if(dist < 250) {{
                        f.targetX = targetX + (Math.random() - 0.5) * 20;
                        f.targetY = targetY + (Math.random() - 0.5) * 20;
                    }}
                }});
            }}
            ...
            function evaluateZone() {{
                ...
                let hitChance = insideStrike ? 0.70 : 0.15;
                ...
                if (Math.random() < hitChance) {{
                    ...
                    if (runnerRule === 2 && Math.random() < 0.6) {{
                        hitBall.vx = (Math.random() - 0.5) * 3;
                        hitBall.vy = -(2 + Math.random() * 2);
                        document.getElementById('commentary').innerText = "🎙️ 번트 성공! 내야수들이 전진하여 공을 처리하러 움직입니다!";
                    }} else {{
                        hitBall.vx = (Math.random() - 0.5) * 14;
                        hitBall.vy = -(6 + Math.random() * 11);
                        document.getElementById('commentary').innerText = "🎙️ 깡! 배트 정면에 맞은 타구가 강하게 야수 정면으로 날아갑니다!";
                    }}
                    
                    triggerFieldingAI(hitBall.x + hitBall.vx * 20, hitBall.y + hitBall.vy * 20);
                }} else {{
            ...
            function drawScene() {{
                ...
                let currentFrameTime = Date.now() * 0.01;
                ...
                if (currentView === "catcher") {{
                    ...
                    // 야수 AI 보간 기동 연산
                    fielders.forEach(f => {{
                        f.x += (f.targetX - f.x) * 0.08; f.y += (f.targetY - f.y) * 0.08;
                        ctx.fillStyle = "#3b82f6"; ctx.beginPath(); ctx.arc(f.x, f.y, 7, 0, Math.PI*2); ctx.fill();
                    ...
                    if (batterSwing.active) {{
                        let swingAngle = (batterSwing.frame / 15) * Math.PI * 0.6;
                        ctx.rotate(swingAngle);
                        batterSwing.frame++;
                        if(batterSwing.frame > 15) {{ batterSwing.active = false; }}
                    }}
                    ctx.fillStyle = "#b45309";
                    ctx.fillRect(0, -5, 65, 10); // 배트 형상
                    ctx.restore();
                    ...
                    ctx.strokeStyle = "rgba(220, 38, 38, 0.5)"; ctx.lineWidth = 3; ctx.strokeRect(230, 260, 140, 140);
                }}

                if (isDragging) {{
                    ctx.strokeStyle = "#34d399"; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(dragTarget.x, dragTarget.y, 10, 0, Math.PI*2); ctx.stroke();
                }}
                ...
                if (ball.active && ball.status === "flying") {{
                    ...
                    if (currentView === "catcher") {{
                        ...
                        let r = Math.max(2, 2 + (ball.z * 20));
                        drawRealBaseball(ball.x, ball.y, r, currentFrameTime);
                    }}

                    // ⚠️ 버그 해결 지점: 100% 도달 시 정지 없이 즉시 판정 구역으로 바인딩
                    if (ball.z >= 1.0) {{
                        ball.z = 1.0;
                        evaluateZone();
                    }}
                ...
                if (hitBall.active) {{
                    ...
                    drawRealBaseball(hitBall.x, hitBall.y, 6, currentFrameTime * 2);

                    if (hitBall.y < 40 || hitBall.x < 0 || hitBall.x > 600) {{
                        hitBall.active = false;
                        if (hitBall.y < 40 && hitBall.x > 180 && hitBall.x < 420) {{
                            document.getElementById('commentary').innerText = "🎙️ 홈런!! 담장을 훌쩍 넘어가는 초대형 홈런이 터집니다!";
                            game.s = 0; game.b = 0;
                        }} else {{
                            document.getElementById('commentary').innerText = "🎙️ 파울 플라이 혹은 파울 라인을 벗어난 타구입니다.";
                        }}
                        resetToReady();
                    }}
                }}
                ...
                if (umpSignal.frame > 0) {{
                    ctx.fillStyle = umpSignal.color; ctx.font = "bold 46px sans-serif"; ctx.fillText(umpSignal.text, 205, 115); umpSignal.frame--;
                }}
